...
|---|---|---|
| Kafka, MinIO, PostgreSQL, Spark master/worker | `healthcheck` Docker (`docker compose ps`) | Statut `unhealthy` après 5 tentatives |
...
Toutes les alertes applicatives sont centralisées dans la table `pipeline_alertes` (PostgreSQL), consultable en SQL par toute l'équipe et destinée à alimenter un futur tableau de bord de supervision (cf. pistes d'amélioration).
...
## 5. Pistes d'amélioration (au-delà du MVP)
...
| Stack de monitoring dédiée (Prometheus/Grafana ou ELK) | Visualiser les métriques d'infrastructure (CPU, mémoire, lag Kafka) en complément des alertes applicatives |
...
| Orchestrateur de conteneurs (Kubernetes) avec autoscaling | Absorber une montée en charge (plus de stations, plus de villes) sans intervention manuelle |
| Tests automatisés (CI) sur le schéma Spark et les règles de qualité | Détecter une rupture de contrat d'API avant la mise en production |
