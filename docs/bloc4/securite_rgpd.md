...
Toute la plateforme (Kafka x3, MinIO x4, PostgreSQL primary/replica, Spark, ainsi que la stack de supervision Prometheus/Grafana) est **auto-hébergée en local via Docker Compose**, sur une machine physiquement située en France pour la durée du projet. La duplication des données entre nœuds (réplication Kafka, erasure coding MinIO, streaming replication PostgreSQL, cf. [architecture.md section 5](architecture.md#5-cluster-tolérant-aux-pannes-zéro-panne)) reste strictement locale à ce même poste — la haute disponibilité n'introduit aucun transfert de données hors UE. En cas de migration vers un hébergement cloud (piste d'évolution), le choix se porterait sur une région UE d'un fournisseur (ex. `eu-west-3` Paris chez AWS, ou un hébergeur français comme OVHcloud/Scaleway) afin de conserver cette garantie.
...
- Aucun identifiant/mot de passe n'est écrit en dur dans le code ou dans `docker-compose.yml` : toutes les valeurs sensibles (`MINIO_ROOT_PASSWORD`, `POSTGRES_PASSWORD`, `POSTGRES_REPLICATOR_PASSWORD`, `GRAFANA_ADMIN_PASSWORD`, etc.) sont lues depuis un fichier `.env` **non versionné** (cf. `.gitignore`), à partir du modèle fourni dans `.env.example`. Le rôle `POSTGRES_REPLICATOR_USER` dédié à la réplication (cf. `sql/init_replication.sh`) n'a que le privilège `REPLICATION` — pas d'accès aux tables applicatives.
- Même principe côté Kubernetes : les secrets (`k8s/02-secret.example.yaml`) suivent le même modèle gabarit/`.gitignore` que `.env.example`, jamais committés une fois renseignés (cf. [mspr-tech/k8s/README.md](../../mspr-tech/k8s/README.md)).
